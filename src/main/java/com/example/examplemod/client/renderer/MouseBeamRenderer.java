...
public class MouseBeamRenderer extends EntityRenderer<MouseBeamEntity> {
    ...
    public MouseBeamRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public boolean shouldRender(MouseBeamEntity entity, Frustum camera, double camX, double camY, double camZ) {
        return true;
    }

    @Override
    public void render(MouseBeamEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
    ...
    private void renderBeam(MouseBeamEntity entity, Vec3 start, Vec3 end, float partialTicks, PoseStack poseStack, MultiBufferSource buffer) {
        ...
        float textureOffset = -(entity.tickCount + partialTicks) * 0.9F;
        ...
        renderCylinder(poseStack, coreConsumer, 1.0F, 1.0F, 1.0F, 0.42F, length, 0.025F, textureOffset, textureOffset + length);

        VertexConsumer glowConsumer = buffer.getBuffer(RenderType.beaconBeam(BEAM_LOCATION, false));
        renderCylinder(poseStack, glowConsumer, 1.0F, 1.0F, 1.0F, 0.08F, length, 0.055F, textureOffset, textureOffset + length);
        poseStack.popPose();
    }
    ...
    private static void renderCylinder(PoseStack poseStack, VertexConsumer consumer, float red, float green, float blue, float alpha,
                                       float length, float radius, float minV, float maxV) {
    ...
    @Override
    public ResourceLocation getTextureLocation(MouseBeamEntity entity) {
