...
3.  **模型文件 (Models)**:
    *   **Item Model**: `src/main/resources/assets/chen_mod/models/item/xxx_talisman.json`
    *   内容模板：
        ```json
        {
          "parent": "minecraft:item/generated",
          "textures": {
            "layer0": "chen_mod:item/xxx_talisman"
          }
        }
        ```
...
*   **必须使用中文**: 所有新增代码（包括类、方法、字段）必须包含详细的中文 Javadoc 注释，解释其功能、参数和逻辑。
*   **关键逻辑注释**: 对于复杂的业务逻辑（如融合算法、AI目标选择），必须在代码内部添加详细的中文行内注释。
*   **禁止使用 HTML 标签**: 在 Javadoc 中严禁使用 `<p>`、`<br>` 等 HTML 标签。请使用空行来分隔段落。
    *   **错误示例**:
        ```java
        /**
         * 第一段。
